 ...
 *   B1       3       B        1
 *
 * A list of profiles contains different FSRC, DUC and DDC configurations. Individual channels (per side) may
 ...
 * - For onboard SYSREF generation and alignment using MCS calibration, example can be run with command line option '-mcs'
 *
...
static int32_t apollo_fsrc_config(adi_apollo_device_t* device, uint8_t subclass, double dev_clk_freq, double sysref_freq, bool use_mcs_cal);
static int32_t fpga_fsrc_config(adi_apollo_device_t *device, adi_fpga_apollo_device_t *fpga_device, adi_apollo_top_t *profile, uint8_t subclass,
                                adi_ads10_apollo_dp_info_t *rx_dp_info, adi_ads10_apollo_dp_info_t *tx_dp_info, bool use_mcs_cal);
...
int32_t fullchip_fsrc_sc1_ext_trig(adi_apollo_device_t *device, adi_fpga_apollo_device_t *fpga_device, adi_apollo_top_t *profile, int argc, char *argv[], int argc_ofst)
{
    ...
    bool use_mcs_cal = false;   // cmd arg: -mcs. True for onboard SYSREF generation and alignment, False if SYSREF is from sig-gen and HW alignment is required
    ...
    for (uint8_t i = argc_ofst; i < argc; ++i) {
        if (strncmp(argv[i], "-i", 3) == 0) {
            interactive = true;
        } else if (strncmp(argv[i], "-mcs", 5) == 0) {
            use_mcs_cal = true;
        }
    }
    ...
    if (fpga_device->state_info.design_id != ADI_FPGA_APOLLO_DESIGN_MCS) {
        printf("This example only runs with MCS type FPGA images.\n");
     ...
     */
    err = apollo_fsrc_config(device, subclass, rx_dp_info[0].fclk, sysref_freq, use_mcs_cal);
    ADI_CMS_ERROR_GOTO(err, end);

    /*
     * Configure FPGA for FSRC
     */
    err = fpga_fsrc_config(device, fpga_device, profile, subclass, rx_dp_info, tx_dp_info, use_mcs_cal);
    ADI_CMS_ERROR_GOTO(err, end);
    ...
    if (use_mcs_cal) {
        ...
        printf("Using ADF4030 for SYSREF generation and MCS Cal(SW) for its alignment.\n");
        err = adi_ads10_apollo_ex_mcs_sysref_gen_align_cal_setup(device, fpga_device, profile);
        ADI_CMS_ERROR_RETURN(err);
    } else {
        /* SYSREF alignment (HW method) */
        printf("Using Sig-gen for SYSREF generation and Oneshot Sync(HW) for its alignment.\n");
    ...
    /* Print ext sysref to internal sysref phase delta
       Note: This delta should be ~0 after HW sync alignment and will be ~40 if MCS Sync is used for alignment. */
...
static int32_t apollo_fsrc_config(adi_apollo_device_t* device, uint8_t subclass, double dev_clk_freq, double sysref_freq, bool use_mcs_cal)
{
    ...
    err = adi_apollo_clk_mcs_trig_sync_enable(device, 0);         // Disable trig sync, will get enabled later
    ADI_CMS_ERROR_RETURN(err);
    err = adi_apollo_clk_mcs_trig_reset_disable(device);
    ADI_CMS_ERROR_RETURN(err);
...
#include "adi_apollo_bf_jtx_dual_link.h"
    ...
    err = adi_apollo_clk_mcs_sync_trig_map(device, ADI_APOLLO_RX_TX_ALL, ADI_APOLLO_TRIG_PIN_A0);
    ADI_CMS_ERROR_RETURN(err);

    if (!use_mcs_cal) {
        /* Set the subclass mode for JESD JRx and JTx links */
        err = adi_apollo_jrx_subclass_set(device, ADI_APOLLO_LINK_ALL, subclass);
        ADI_CMS_ERROR_RETURN(err);
        err = adi_apollo_jtx_subclass_set(device, ADI_APOLLO_LINK_ALL, subclass);
        ADI_CMS_ERROR_RETURN(err);

        /* Set the subclass mode for Apollo MCS */
        err = adi_apollo_clk_mcs_subclass_set(device, subclass);
        ADI_CMS_ERROR_RETURN(err);
        ...
        *
        */
        err = adi_apollo_clk_mcs_internal_sysref_per_set(device, (uint16_t) ((dev_clk_freq / sysref_freq / 8) + 0.5));
        ADI_CMS_ERROR_RETURN(err);

        /* Enable the MCS SYSREF receiver if subclass 1 */
        err = adi_apollo_clk_mcs_sysref_en_set(device, (subclass == 1) ? ADI_APOLLO_ENABLE : ADI_APOLLO_DISABLE);
        ADI_CMS_ERROR_RETURN(err);
    }
                                ...
                                adi_ads10_apollo_dp_info_t* rx_dp_info, adi_ads10_apollo_dp_info_t* tx_dp_info, bool use_mcs_cal)
{
     ...
     */
    err = adi_fpga_apollo_core_jtx_link_cnt_get(fpga_device, &fpga_link_count);
    ADI_CMS_ERROR_RETURN(err);
     ...
     */
    err = adi_fpga_apollo_private_write32_bitfield(fpga_device, JESD204B_RX_CONFIG, JESD204B_RX_SUBCLASS_MASK, subclass);
    ADI_CMS_ERROR_RETURN(err);
    err = adi_fpga_apollo_private_write32_bitfield(fpga_device, JESD204B_TX_CONFIG, JESD204B_TX_SUBCLASS_MASK, subclass);
    ADI_CMS_ERROR_RETURN(err);
    ...
    /*
     * Select ADS10 external SYSREF source
     * Input from FMC, if using on-board SYSREF generation. i.e. use_mcs_cal = true
     * Input on SMA J5, if using SYSREF from Sig-gen. i.e. use_mcs_cal = false
     */
    err = adi_fpga_apollo_core_sysref_src_set(fpga_device, use_mcs_cal ? ADI_FPGA_APOLLO_SYSREF_SRC_FMC : ADI_FPGA_APOLLO_SYSREF_SRC_EXT);
    ADI_CMS_ERROR_RETURN(err);
     ...
     * GPIO
     */
    err = adi_fpga_apollo_private_write32_bitfield(fpga_device, GPIO_D_1, DUT_GPIO_D_1_MASK, 0);
    ADI_CMS_ERROR_RETURN(err);
    err = adi_fpga_apollo_private_write32_bitfield(fpga_device, GPIO_D_2, DUT_GPIO_D_2_MASK, 0);
    ADI_CMS_ERROR_RETURN(err);
    err = adi_fpga_apollo_private_write32_bitfield(fpga_device, GPIO_W_1, DUT_GPIO_W_1_MASK, 0);
    ADI_CMS_ERROR_RETURN(err);
    err = adi_fpga_apollo_private_write32_bitfield(fpga_device, GPIO_W_2, DUT_GPIO_W_2_MASK, 0);
    ADI_CMS_ERROR_RETURN(err);
    ...
    */
    ...
    err = adi_fpga_apollo_hw_fsrc_tx_sequencer_config(fpga_device, &sysref_seq_cnt);
    ADI_CMS_ERROR_RETURN(err);
...
static int32_t reconfig_setup(adi_apollo_device_t* device, adi_fpga_apollo_device_t* fpga_device, rx_tx_reconfig_profile_t* rec_profile, adi_ads10_apollo_dp_info_t* tx_dp_info)
{
     ...
     *   B1       3       B        1
    */
    adi_apollo_blk_sel_t cddc_sel[MAX_CHANS] = {ADI_APOLLO_CDDC_A0, ADI_APOLLO_CDDC_A1, ADI_APOLLO_CDDC_B0, ADI_APOLLO_CDDC_B1};
    adi_apollo_blk_sel_t fddc_sel[MAX_CHANS] = {ADI_APOLLO_FDDC_A0 | ADI_APOLLO_FDDC_A1, ADI_APOLLO_FDDC_A2 | ADI_APOLLO_FDDC_A3,
                                                ADI_APOLLO_FDDC_B0 | ADI_APOLLO_FDDC_B1, ADI_APOLLO_FDDC_B2 | ADI_APOLLO_FDDC_B3};

    adi_apollo_blk_sel_t cduc_sel[MAX_CHANS] = {ADI_APOLLO_CDUC_A0, ADI_APOLLO_CDUC_A1, ADI_APOLLO_CDUC_B0, ADI_APOLLO_CDUC_B1};
    adi_apollo_blk_sel_t fduc_sel[MAX_CHANS] = {ADI_APOLLO_FDUC_A0 | ADI_APOLLO_FDUC_A1, ADI_APOLLO_FDUC_A2 | ADI_APOLLO_FDUC_A3,
                                                ADI_APOLLO_FDUC_B0 | ADI_APOLLO_FDUC_B1, ADI_APOLLO_FDUC_B2 | ADI_APOLLO_FDUC_B3};

    for (int k = 0; k < MAX_CHANS; k++) {
        /* CDDC/FDDC */
        err = adi_apollo_cddc_dcm_set(device, cddc_sel[k], rec_profile->rx_chan[k]->cddc_dcm);
        ADI_CMS_ERROR_RETURN(err);
        err = adi_apollo_fddc_dcm_set(device, fddc_sel[k], rec_profile->rx_chan[k]->fddc_dcm);
        ADI_CMS_ERROR_RETURN(err);


        /* CDUC/FDUC */
        err = adi_apollo_cduc_interp_set(device, cduc_sel[k], rec_profile->tx_chan[k]->cduc_interp);
        ADI_CMS_ERROR_RETURN(err);
        err = adi_apollo_fduc_interp_set(device, fduc_sel[k], rec_profile->tx_chan[k]->fduc_interp);
        ADI_CMS_ERROR_RETURN(err);
    }
...
static __maybe_unused int32_t fpga_create_and_load_vec(adi_fpga_apollo_device_t* fpga_device, adi_apollo_top_t *profile, vec_type_e vec_type)
{
    ...
    if (vec_type == PULSE_10) {
        err = adi_ads10_apollo_ex_vec_pulse_write(fpga_device, profile, NULL, ADI_APOLLO_LINK_ALL, 8192, 10.0/8192, 0.5, 0.5);
    } else if (vec_type == TONE_0P40_FS) {
        err = adi_ads10_apollo_ex_vec_cmplx_tone_write(fpga_device, profile, NULL, ADI_APOLLO_LINK_ALL, SAMPLES_PER_VC, 0.4, -9.0);
    } else if (vec_type == TONE_0P05_FS) {
        err = adi_ads10_apollo_ex_vec_cmplx_tone_write(fpga_device, profile, NULL, ADI_APOLLO_LINK_ALL, SAMPLES_PER_VC, 0.05, -9.0);
    } else if (vec_type == TONE_0P015625_FS) {
        err = adi_ads10_apollo_ex_vec_cmplx_tone_write(fpga_device, profile, NULL, ADI_APOLLO_LINK_ALL, SAMPLES_PER_VC, 0.015625, -9.0);
    } else {
