...
int32_t rx_jesd(adi_apollo_device_t *device, adi_fpga_apollo_device_t *fpga_device, adi_apollo_top_t *profile, int argc, char *argv[], int argc_ofst)
{
    ...
    */
    ...
    f_cnco_mhz = ((f_adc_mhz)*profile->rx_path[0].rx_cddc[0].nco[0].nco_phase_inc) / (1ull << 32);
    f_fnco_mhz = ((f_adc_mhz / cddc_dcm) * profile->rx_path[0].rx_fddc[0].nco[0].nco_phase_inc) / (1ull << 48);
    ...
    printf("%-12s %-7s %-5s %-11s %-11s %-4s %-21s %-8s %-8s\n", "Profile     ", "Fdac   ", "PFILT", "CDCU       ", "FDUC       ", "CFIR", "FSRC     ", "Fdata   ", "JESD DR ");
    printf("%-12s %-7s %-5s %-11s %-11s %-4s %-21s %-8s %-8s\n", "============", "=======", "=====", "===========", "===========", "====", "=====================", "========", "========");
    ...
    if (bmem_awg) {
        ...
        ADI_CMS_ERROR_RETURN(err);
        err = adi_ads10_apollo_ex_bmem_awg_tone_write(device, bmem_sel, f_adc_in_tone/f_adc_mhz, 0, false);
        ADI_CMS_ERROR_RETURN(err);
    }
    else {
    ...
    EXCTL_RX_MEAS_NSD(0x33, cap_fname_base, bmem_awg ? -160 : -145, bmem_awg ? 7 : 3); // C# analysis library has issues with uc08a/uc08a1 bmem NSD
