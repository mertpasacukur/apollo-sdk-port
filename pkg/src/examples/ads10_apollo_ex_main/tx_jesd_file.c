...
int32_t tx_jesd_file(adi_apollo_device_t *device, adi_fpga_apollo_device_t *fpga_device, adi_apollo_top_t *profile, int argc, char *argv[], int argc_ofst)
{
    ...
    uint8_t is_auto = (n_loc_args > 0) && (!strcmp("-a", argv[argc_ofst]) || !strcmp("-al", argv[argc_ofst]));
    uint8_t is_long = (n_loc_args > 0) && !strcmp("-al", argv[argc_ofst]);
    ...
    if (is_auto) {
        FILE *fp;
        uint32_t n_samples = is_long ? 20 * 1024 * 1024 : 64 * 1024;
        ...
        int16_t *vec = (int16_t *)malloc(n_samples * sizeof(int16_t));
        ADI_CMS_MEM_ALLOC_CHECK(vec);
        ...
        fp = fopen(i_file_name, "w+");
        ADI_CMS_FILE_OPEN_CHECK(fp);
        adi_vector_generate_sine(vec, n_samples, &sine_config);
        for (int i = 0; i < n_samples; i++) {
            fprintf(fp, "%d\n", vec[i]);
        }
        fclose(fp);
        
        fp = fopen(q_file_name, "w+");
        ADI_CMS_FILE_OPEN_CHECK(fp);
    ...
    uint32_t vec_len;
    err = adi_apollo_utilities_file_sample_count(device, i_file_name, 0, &vec_len);
    ADI_CMS_ERROR_RETURN(err);
    
    err = adi_ads10_apollo_ex_vec_files_write(fpga_device, profile, NULL, ADI_APOLLO_LINK_ALL, vec_len, i_file_name, q_file_name, 1.0);
    ADI_CMS_ERROR_RETURN(err);
