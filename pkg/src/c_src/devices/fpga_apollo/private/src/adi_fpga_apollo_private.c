...
int32_t adi_fpga_apollo_private_jesd_rx_phy_prbs_lanes_set(adi_fpga_apollo_device_t *fpga)
{
	...
	err = adi_fpga_apollo_private_write32_bitfield(fpga,
		JESD204B_RX_L_SCR,
		JESD204B_LINKX_RX_L_PARAM_MASK << 0,
		11); /* 12 lanes side A link 0 */
	ADI_CMS_ERROR_RETURN(err);

	if (fpga->state_info.jrx_link_count == 4) {
		err = adi_fpga_apollo_private_write32_bitfield(fpga,
			JESD204B_RX_L_SCR,
			JESD204B_LINKX_RX_L_PARAM_MASK << 8,
			0); /* 0 lanes side A link 1 */
		ADI_CMS_ERROR_RETURN(err);
		err = adi_fpga_apollo_private_write32_bitfield(fpga,
			JESD204B_RX_L_SCR,
			JESD204B_LINKX_RX_L_PARAM_MASK << 16,
			11); /* 12 lanes side B link 0 */
		ADI_CMS_ERROR_RETURN(err);
	} else if (fpga->state_info.jrx_link_count == 2) {
		err = adi_fpga_apollo_private_write32_bitfield(fpga,
			JESD204B_RX_L_SCR,
			JESD204B_LINKX_RX_L_PARAM_MASK << 8,
			11); /* 0 lanes side B link 0 */
		ADI_CMS_ERROR_RETURN(err);
		err = adi_fpga_apollo_private_write32_bitfield(fpga,
			JESD204B_RX_L_SCR,
			JESD204B_LINKX_RX_L_PARAM_MASK << 16,
	...
	err = adi_fpga_apollo_private_write32_bitfield(fpga,
		JESD204B_RX_L_SCR,
		JESD204B_LINKX_RX_L_PARAM_MASK << 24,
		0); /* 0 lanes side B link 1 */
	ADI_CMS_ERROR_RETURN(err);
