...
/*!
 * \brief Defines a function pointer that generates a vector (e.g. sine)
 */
typedef int32_t (*adi_vector_generator)(int16_t samples[], uint32_t sample_count, void *config);

/*!
 * \brief Defines a function pointer that is called before generating a vector
 */
typedef int32_t (*adi_vector_generator_init)(void *config);

/*!
 * \brief Defines a function pointer is called after generating a vector
 */
typedef int32_t (*adi_vector_generator_deinit)(void *config);

typedef struct {
    adi_vector_generator generate;
    adi_vector_generator_init init;
    adi_vector_generator_deinit deinit;
} adi_vector_loader_t;
...
typedef struct {
    ...
    uint32_t sample_total;  /*< Total vector len (sum of chunked vector) */
    uint32_t sample_idx;    /*< sample index when creating vec iteratively in chunks */
...
typedef struct{
    ...
    int16_t step;           /*< Step between samples */
    uint32_t sample_total;  /*< Total vector len (sum of chunked vector) */
    uint32_t sample_idx;    /*< sample index when creating vec iteratively in chunks */
...
/*!
 * \brief Constant vector parameters
 */
typedef struct
{
    int16_t value;          /*< Constant sample value */
    uint32_t sample_total;  /*< Total vector len (sum of chunked vector) */
    uint32_t sample_idx;    /*< sample index when creating vec iteratively in chunks */
} adi_vector_constant_config_t;
 ...
 * @param config constant value config pointer  \ref adi_vector_constant_config_t
 *
 ...
 * @param sample_count max number of samples to load
 * @param config tone config \ref adi_ads10_apollo_tone_config_t
 * @param tone_ratio_out The actual coherent tone ratio used
 *
 * @return 0                     				Vector successfully created
 * @return <0                                   Failed.
 */
 ...
 * @param samples array containing generated samples
 * @param sample_count max number of samples to load
 * @param config ramp config \ref adi_vector_ramp_config_t
 *
 * @return 0                     				Vector successfully created
 * @return <0                                   Failed.
 */
...
/**
 * @brief   Called before samples are read from a file.
 * 
 * @note    This function is typically used to open a file.
 *
 * @param config tone config \ref adi_vector_file_config_t
 *
 * @return 0                                    Init executed ok
 * @return <0                                   Failed.
 */
int32_t adi_vector_file_init(void *config);

/**
 * @brief   Called after all samples are read from a file.
 * 
 * @note    This function is typically used to close a file.
 *
 * @param config tone config \ref adi_vector_file_config_t
 *
 * @return 0                                    Deinit executed ok
 * @return <0                                   Failed.
 */
 ...
 * @brief   Nop function
 * 
 * @note    This function is used for vec gens that don't need init or deinit operations.
 *
 * @param config void *
 *
 * @return 0                                    Always
